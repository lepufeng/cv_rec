...
> 后端服务：上传简历 → 多模态模型解析为结构化 JSON → 浏览器插件提交目标网页表单字段后，返回智能填写方案。
...
不依赖预定义的"标签库"。每次投递时由 LLM 实时理解目标网页的字段语义，从用户的结构化简历中匹配填入。这意味着平台不需要为每个 ATS 维护字段映射规则，新站点零适配。

两个阶段：

- **阶段 A · 简历解析**：用户首次上传简历后异步执行的一次性工作。结果以稳定 Schema 持久化，作为该用户的"简历数据源"。
- **阶段 B · 智能填写**：浏览器插件抓取目标网页表单后调用，每次投递触发一次。LLM 接收（结构化简历 + 表单字段）输入，输出每字段的填写方案与置信度。
...
设计文档已包含完整方案，见 `.kiro/specs/resume-parsing-platform/design.md`。
...
完整 v2 设计见 `.kiro/specs/resume-parsing-platform/design.md`。
...
2. **GLM-4.6V-Flash 主力模型**：限时免费。压测显示阶段 A 单份 ≈ 0.01 元、阶段 B 单次 ≈ 0.005 元（用付费版本估算），全优化后毛利率 > 97%。详见 `cost_experiment.py`。
3. **不预定义"标签库"**：差异化卖点。表单字段语义由 LLM 即时推理，新 ATS 零适配成本。
