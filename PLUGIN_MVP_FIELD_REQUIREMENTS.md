# 小鹏/飞书招聘插件字段扫描上报要求（MVP）

> 面向插件开发同学。  
> 当前目标：先跑通“小鹏/飞书招聘页面扫描 → 平台生成填写方案 → 插件执行填写”的 MVP。
...
插件入口层应先做支持范围判断。当前允许小鹏招聘、`*.jobs.feishu.cn`、已确认的飞书招聘公司域名，以及带有 Feishu/Formily/UD 组件特征的招聘表单；其他站点直接提示不支持，不再上报给平台尝试泛化填写。
