...
【多行文本格式】
- 项目经历、实习经历、工作经历、校园经历中的“描述 / 内容 / 职责 / 成果 / 业绩”等 textarea 或富文本字段，如果来源是 achievements / bullet 列表，必须每条一行，用 "\\n" 连接
- 不要把 bullet 列表用 "、"、"；" 或 "。" 串成一整段；例如输出 "完成A\\n完成B"，不要输出 "完成A。、完成B"
- 技能栈、工具列表、城市偏好等非描述类短字段仍可用 "、" 连接
...
- "兴趣爱好" ⟷ "爱好" / "hobbies"
- "项目经验" ⟷ "项目经历" / "实战项目"
- "开发语言" / "编程语言" ⟷ skills.programming_languages；但 "外语考试/等级" / "语言能力等级" ⟷ languages[].level，不能填 Python / SQL / R / Tableau 这类技能
...
【复合字段拆分】
- 手机号码若被拆成“国家/地区/区号 + 号码”两个控件：区号控件只填国家码或最匹配 option（如 +86 / 中国 +86），号码控件才填完整手机号
- 起止时间若被拆成两个 groupIndex：groupIndex=0 填开始时间，groupIndex=1 填结束时间；单个 date-range 控件可填 "开始 - 结束"

【输出 Schema】
{
  "filled": {
    "<field_id>": {
      "value": str | [str] | [dict],  // 多选/复选可用 list[str]；repeater 时为 list[dict]
